...
public class ExtentReportManager implements ITestListener 
{
	...
	public void onStart(ITestContext testContenxt)
	{
		...
		*/
		...
		//Title of Report
		sparkReporter.config().setDocumentTitle("Automation Test Store Testing Report");
		// Name OF THe Report
		sparkReporter.config().setReportName("Automation Test Store Functional Report");
		sparkReporter.config().setTheme(Theme.DARK);
		
		extent=new ExtentReports();
		extent.attachReporter(sparkReporter);
		extent.setSystemInfo("Application", "AutomationTestStore");// Application Name we Have to Add
	...
	public void onFinish(ITestContext testContext)
	{
		...
		// if you want to share report to the team as test complete then this piece of code will help
		/*
		try
		{
		    ...
		    Path reportPath = Path.of(System.getProperty("user.dir"), "reports", repName);
		    // Convert the path to a URI and then to a URL
		    URI uri = reportPath.toUri();
		    URL url = uri.toURL();
			...
			email.setAuthenticator(new DefaultAuthenticator("email","Password"));   // Add email and password
			email.setSSLOnConnect(true);
			email.setFrom("email");   // Sender
			email.setSubject("Test Result");
			email.setMsg("Please Find Attached Report");
			email.addTo("receiver email");  // Add receiver email Id
